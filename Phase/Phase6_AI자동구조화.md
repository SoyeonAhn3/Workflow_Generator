...
export default async function handler(req, context) {
  ...
  const { userInput, dept } = await req.json()

  if (!userInput || !dept) {
    return new Response(JSON.stringify({ error: '입력값 누락' }), { status: 400 })
  }
  ...
  try {
    const message = await client.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `다음 업무 흐름을 분석하여 JSON 배열만 응답하세요.
...
각 항목: title, screenName, dept, pt, logic, warning`
    ...
    return new Response(
      JSON.stringify({ error: 'AI 응답 오류 또는 JSON 파싱 실패' }),
      { status: 500 }
    )
  }
}
...
| JSON 파싱 실패 처리 | Step 1 복귀 + 안내 메시지 | 사용자가 입력 수정 후 재시도 가능 |
...
- JSON 파싱 실패 시 Step 1 복귀 + 안내 메시지 표시
